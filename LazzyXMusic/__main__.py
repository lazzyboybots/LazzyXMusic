...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("LazzyXMusic.plugins" + all_module)
    LOGGER("LazzyXMusic.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Lazzy.start()
    try:
        await Lazzy.stream_call("https://envs.sh/5Fy.jpg")
    except NoActiveGroupCall:
        LOGGER("LazzyXMusic").error(
            "Please turn on the videochat of your log group\channel.\n\nStopping Bot..."
        )
        exit()
    except:
        pass
    await Lazzy.decorators()
    LOGGER("LazzyXMusic").info(
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("LazzyXMusic").info("Stopping LazzyX Music Bot...")
